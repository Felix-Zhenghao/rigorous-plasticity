...
class Blocks:
    ...
    def __init__(self, lengths=(4,), consume=Consumption(2, epochs=2, shuffle_each_epoch=False), stochastic=False):
...
def test_exact_chunk_order_and_short_batches():
...
def test_exact_update_budget_cycles_per_chunk():
...
def test_resume_committed_cursor_with_prefetch_and_augmentation(workers):
...
def test_uncommitted_data_cannot_advance_or_checkpoint():
    consumer = Consumer(Blocks(), 1)
    next(consumer)
...
def test_arrivals_count_available_chunk_separately_from_exposures():
...
def test_singleton_online_creates_one_view_per_task():
...
def test_fixed_head_loss_and_no_regression_broadcasting():
...
def test_schedule_holds_terminal_and_validates_unused_fields():
