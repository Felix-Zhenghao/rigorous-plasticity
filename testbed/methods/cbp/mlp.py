...
def bind_sites(network: MLP, config: CBPConfig) -> dict[str, RecyclingSite]:
    ...
    if getattr(config, "bias_compensation", False):
        if config.statistic_site != "after_activation" or any(layer.norm_position != "pre_activation" or layer.dropout.p for _, layer in layers):
